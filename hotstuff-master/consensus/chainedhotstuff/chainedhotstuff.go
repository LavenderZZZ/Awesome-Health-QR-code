...
func (hs *ChainedHotStuff) CommitRule(block *consensus.Block) *consensus.Block {
	block1, ok := hs.qcRef(block.QuorumCert()) //证书指向的block
	if !ok {
		return nil
	}
...
func (hs *ChainedHotStuff) VoteRule(proposal consensus.ProposeMsg) bool {
	...
	if haveQCBlock && qcBlock.View() > hs.bLock.View() {
		...
		if hs.mods.BlockChain().Extends(block, hs.bLock) { //checks if the given block extends the branch of the target hash.
			safe = true
		} else {
