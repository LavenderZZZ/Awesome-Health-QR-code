...
func (cs *consensusBase) OnPropose(proposal ProposeMsg) {
	...
	cs.mods.Logger().Infof("OnPropose: block is accepted\n block hash is %s\n  block cmd is %s\n", proposal.Block.hash.String(), proposal.Block.cmd)
	cs.mods.Logger().Info("the block chain now is :")
	tmphash := block.hash
	for {
		b, _ := cs.mods.BlockChain().LocalGet(tmphash)
		cs.mods.Logger().Infof("Parent Hash is %s\n Block hash is %s\n  block cmd is %s\n", b.parent.String(), b.hash.String(), b.cmd)
		if b.view == 1 {
			cs.mods.Logger().Info("Output block chain is over:")
			break
		}
		tmphash = b.Parent()
	}
